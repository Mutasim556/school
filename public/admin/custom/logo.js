...
$('#images1').on('change', function () { 
    // alert('dddd');
    $('#upload-demo-div1').removeClass('d-none')
    var reader = new FileReader();
      reader.onload = function (e) {
...
$('#school_content_form').submit(function(e){
    e.preventDefault();
    resize1.croppie('result', {
        ...
        $.ajax({
            type : 'POST',
            url : 'upload-school-logo', 
            data: {"school_logo":img,"school_name":$("input[name=school_name]").val(),"school_eiin":$("input[name=school_eiin]").val(),"school_founded":$("input[name=school_founded]").val(),"type":$('#type').val(),"text":$('#text').val()},
            ...
            success : function(data){
                Swal.fire({
                    type: 'success',
                    title: '',
                    text: 'School content uploaded success',
                    showConfirmButton: false,
                    timer:1500,
                }).then(()=>{
                    $('#upload_image').addClass(' d-none')
        ...
        success : function(data){
            ...
            $.each(data,function(key,value){
                if(value.logo_type=='image'){
                    url = 'http://localhost/school/'+value.logo_image;
                    content = '<img src="'+url+'">'
