...
class LogoController extends Controller
{
    public function GetLogo(){
        ...
        return view('admin.logo.logo',compact('school_contents'));
    }
        ...
        if($data->type=='image'){
            ...
            if($data->position=='admin_top'){
                if($height!=60 || $width!=92){
                    return response()->json(['message'=>'Admin top image must be 92x60'],422);
                }else{
                    $file = $data->image;
                    ...
                    $path = public_path('files/web_images/'.$file_name);
                ...
                if($height!=60 || $width!=92){
                    return response()->json(['message'=>'Admin bottom image must be 92x60'],422);
                }else{
                    $file = $data->image;
    ...
    public function UploadSchoolLogo(Request $data){
        $data->validate([
            'school_name'=>'required|max:255',
            'school_eiin'=>'required|max:20',
            'school_founded'=>'required|max:20',
            'school_logo'=>'required',
        ]);
         $file = $data->school_logo;
         if($file){
            list($type, $file) = explode(';', $file);
            list(, $file)      = explode(',', $file);
            ...
            $file = base64_decode($file);
            $file_name= time().'.png';
            $path = public_path('school/upload/logo/'.$file_name);
 
            file_put_contents($path, $file);
            $update = DB::table('school_contents')->where('school_content_id',1)->update([
                'school_name'=>$data->school_name,
                'school_eiin'=>$data->school_eiin,
                'school_founded'=> $data->school_founded,
                'school_logo' => 'public/school/upload/logo/'.$file_name,
            ]);
            if($update){
                return $update;
            }else{
            ...
            if($update){
                ...
                return response()->json(['message'=>'Server error 2'],422); 
            }
         }
    }
}
